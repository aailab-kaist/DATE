...
class AttnProcessor:
    def __call__(
        ...
        if save:
            if 77 in attention_probs.shape:
                np.save(f'{save}_{round(time.time(), 3):.3f}_{str(attention_probs.shape)}.npy', attention_probs.detach().cpu().numpy().astype(np.float16))
