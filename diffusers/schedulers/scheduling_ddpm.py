...
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(self, num_inference_steps: int, device: Union[str, torch.device] = None):
        ...
        self.timesteps = torch.cat([self.timesteps, torch.zeros_like(self.timesteps[:1])])
    ...
    def _get_variance(self, t, predicted_variance=None, variance_type=None):
        ...
        if prev_t.size != 1:
            ...
            bool_prev = (prev_t >= 0)
            alpha_prod_t_prev = (torch.ones_like(alpha_prod_t) * self.one)
            alpha_prod_t_prev[bool_prev] = self.alphas_cumprod[prev_t[bool_prev]][:, None, None, None]

            alpha_prod_t = alpha_prod_t.cuda()
            alpha_prod_t_prev = alpha_prod_t_prev.cuda()
        else:
    ...
    def step(
        ...
        t = self.timesteps[timestep_idx].cpu().numpy()
        prev_t = self.timesteps[timestep_idx + 1].cpu().numpy()
        ...
        # print(timestep_idx, self.timesteps[timestep_idx].cpu().numpy(), self.timesteps[timestep_idx+1].cpu().numpy(),
        ...
        if t.size == 1:
            if t > 0:
                ...
                if self.variance_type == "fixed_small_log":
                    ...
                    #print(self._get_variance(t, predicted_variance=predicted_variance))
            ...
            if bool_t.sum() > 0:
                ...
                if predicted_variance is not None:
                    predicted_variance = predicted_variance[bool_t]
                if self.variance_type == "fixed_small_log":
                    variance = self._get_variance(t[bool_t], predicted_variance=predicted_variance) * variance_noise[bool_t]
                elif self.variance_type == "learned_range":
                    variance = self._get_variance(t[bool_t], predicted_variance=predicted_variance)
                    variance = torch.exp(0.5 * variance) * variance_noise[bool_t]
                else:
                    #print(self._get_variance(t, predicted_variance=predicted_variance))
                    variance = (self._get_variance(t[bool_t], predicted_variance=predicted_variance) ** 0.5) * variance_noise[bool_t]
                try:
                    pred_prev_sample[bool_t] = pred_prev_sample[bool_t] + variance
                except:
                    import pdb
                    pdb.set_trace()
