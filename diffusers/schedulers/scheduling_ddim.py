...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        #print("effective sigma:")
        effective_sigma = torch.sqrt((1 - self.alphas_cumprod) / self.alphas_cumprod)
        #print(effective_sigma)
        idx = torch.argmin(abs(effective_sigma - 0.9))
        #print("idx:", idx)
    ...
    def _convert_to_karras(self, in_sigmas: torch.FloatTensor, num_inference_steps) -> torch.FloatTensor:
        ...
        rho = 5.0  # 7.0 is the value used in the paper
    ...
    def set_timesteps(self, num_inference_steps: int, device: Union[str, torch.device] = None):
        ...
        if self.use_sigma:
            ...
            timesteps_ = np.array([self._sigma_to_t(sigma, log_sigmas) for sigma in sigmas])
            sigmas = np.concatenate([sigmas, [0.0]]).astype(np.float32)
            # self.sigmas = torch.from_numpy(sigmas).to(device)
            self.timesteps = torch.round(torch.from_numpy(timesteps_).to(self.timesteps.device)).int()

        #print(self.timesteps)
        #print((1 - self.alphas_cumprod[self.timesteps]) / self.alphas_cumprod[self.timesteps])
    ...
    def step(
        ...
        # print(f"{timestep} -> {prev_timestep}")
        ...
        if type(timestep_idx) == int:
            ...
            alpha_prod_t = self.alphas_cumprod[self.timesteps[timestep_idx].cpu().numpy()][:, None, None, None].cuda()
            alpha_prod_t_prev = self.alphas_cumprod[self.timesteps[timestep_idx + 1].cpu().numpy()][:, None, None, None].cuda()
        # print(self.timesteps[timestep_idx].cpu().numpy(), '->', self.timesteps[timestep_idx+1].cpu().numpy(),
