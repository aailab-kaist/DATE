...
class ImageReward(nn.Module):
    def __init__(self, med_config, device='cpu'):
        ...
        self.resize_transform = Resize((224, 224))  # Resize to 224x224 for CLIP
    ...
    def prepare_text_input(self, prompt):
        if (type(prompt).__name__ == 'list'):
            text_input = self.blip.tokenizer(prompt, padding='max_length', truncation=True, max_length=35,
                                             return_tensors="pt").to(self.device)
        else:
            text_input = prompt
    ...
    def score_batch(self, prompt, image):
        # text encode
        text_input = self.prepare_text_input(prompt)
        ...
        if isinstance(image[0], Image.Image):
            ...
            for i in range(len(image)):
                pil_image = image[i]
                lst_image.append(self.preprocess(pil_image).unsqueeze(0))  # [b, 3, 224, 224]
            image = torch.cat(lst_image).to(self.device)
        elif isinstance(image[0], str) and os.path.isfile(image[0]):
            lst_image = []
            for i in range(len(image)):
        ...
        # text_output.last_hidden_state: [1, 35, 768]
        txt_features = text_output.last_hidden_state[:,0,:] # (feature_dim)
        rewards = self.mlp(txt_features)[:, 0]
        rewards = (rewards - self.mean) / self.std
    ...
    def inference_rank(self, prompt, generations_list):
        ...
        txt_features = torch.cat(txt_set, 0) # [image_num, feature_dim]
